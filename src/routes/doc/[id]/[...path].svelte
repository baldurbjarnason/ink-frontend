<script context="module">
  ...
  export async function preload ({params, query}) {
    ...
    chapterResource.index = bookData.readingOrder.map(item => item.url).indexOf(chapterResource.url)
    const chapterResponse = await this.fetch(`/api/parse-chapter?chapter=${encodeURIComponent(chapterResource.url)}&index=${chapterResource.index}`)
