<script>
  ...
  function onSelect (event) {
    ...
    return sapper.goto(`/collections/${collection}${newOrder.orderBy}${newOrder.reverse}`)
  }
</script>
<script context="module">
    ...
    try {
      ...
      let books = {items: []}
      if (session.profile) {
        books = await this.fetch(`/api/collections?collection=${collection}&page=1&orderBy=${orderBy}&reverse=${reverse}`, {
        credentials: 'include'})
          .then(response => response.json())
      }
      return { items: books.items, collection, selected: `${orderBy}${reverse === 'false' ? '' : '-reversed'}` };
    } catch (err) {
      console.log(err)
...
<div class="Front">
<div class="ViewConfig">Ordered By <label>
