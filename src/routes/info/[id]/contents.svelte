...
<script context="module">
  export async function preload(page, session) {
    ...
    let contents = {}
    if (session.profile) {
      book = await this.fetch(
        `/api/book?url=${encodeURIComponent(`/${id}`)}`,
        {
          credentials: "include"
        }
      ).then(response => response.json())
      .catch(err => this.error(err));
      if (book.resources && book.resources.find(item => 
    item.rel.includes("contents") || item.rel.includes("ncx"))) {
      const toc = book.resources.find(item => 
    item.rel.includes("contents")) || book.resources.find(item => 
    item.rel.includes("ncx"))
...
{#if sidebar}
  <div class="Sidebar" transition:slide="{{delay: 250, duration: 300 }}" on:introstart="{() => sidebargrid = true}" on:outroend="{() => sidebargrid = false}">
