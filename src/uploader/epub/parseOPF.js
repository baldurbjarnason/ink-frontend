export function parseOPF(opf, opfPath) {
  let book = {
    type: "Publication",
    links: [],
    resources: [],
  ...
  book.resources = Array.from(opfDoc.querySelectorAll("item")).map(item => {
    ...
    return {
      url: getPath(item.getAttribute("href"), opfPath),
      rel,
      id,
...
function getPath(path, opfPath) {
  const opf = new window.URL(opfPath, "http://example.com/");
  // Return the full pathname, sans initial '/' as that confuses the zip
  return decodeURIComponent(
    new window.URL(path, opf).pathname.replace("/", "")
  );
}
